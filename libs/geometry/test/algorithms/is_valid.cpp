...
void geometry_to_svg(Geometry const& geometry, const std::string& case_id)
{
#if defined(TEST_WITH_SVG)
    {
        ...
        filename << "/tmp/svg/" << case_id << ".svg";
        std::ofstream svg(filename.str().c_str());
        bg::svg_mapper<point_type> mapper(svg, 1200, 800);
        mapper.add(geometry);
        mapper.map(geometry, "fill-opacity:0.5;fill:rgb(153,204,0);"
                "stroke:rgb(153,204,0);stroke-width:1");

        // Get self-turns and map them
        ...
        {
            if (turn.method == bg::detail::overlay::method_none)
            {
                continue;
            }

            mapper.map(turn.point, "fill:rgb(255,128,0);stroke:rgb(0,0,100);stroke-width:1", 2);
            std::ostringstream out;
            out << bg::method_char(turn.method)
                << std::setprecision(16) << " " << bg::get<0>(turn.point) << " " << bg::get<1>(turn.point);
            mapper.text(turn.point, out.str(), "fill:rgb(0,0,0);font-family:Arial;font-size:10px", 5, 5);
        }
    }
#else
    boost::ignore_unused(geometry, case_id);
#endif
...
inline void test_open_multipolygons()
{
    ...
    {
        // the polygon has a self-touching point at (7,8),
        // defining an interior ring which is not part of its definition,
