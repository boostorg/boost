...
void test_query()
{
  exec::bulk_guarantee_t result1 = boost::asio::query(Executor(), Param());
  BOOST_ASIO_CHECK(result1 == ExpectedResult());

  Executor ex1 = {};
  exec::bulk_guarantee_t result2 = boost::asio::query(ex1, Param());
  BOOST_ASIO_CHECK(result2 == ExpectedResult());

  const Executor ex2 = {};
  exec::bulk_guarantee_t result3 = boost::asio::query(ex2, Param());
...
void test_constexpr_query()
{
  ...
  constexpr exec::bulk_guarantee_t result1 = boost::asio::query(ex1, Param());
  BOOST_ASIO_CHECK(result1 == ExpectedResult());
...
void test_vars()
{
  BOOST_ASIO_CHECK(s() == exec::bulk_guarantee);
  BOOST_ASIO_CHECK(n1() == exec::bulk_guarantee.unsequenced);
  BOOST_ASIO_CHECK(n2() == exec::bulk_guarantee.sequenced);
  BOOST_ASIO_CHECK(n3() == exec::bulk_guarantee.parallel);
}
