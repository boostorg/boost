...
[TIP]
--
In order to support vector types that can not provide access to elements by reference, the `write_element` / `write_element_idx` static member functions can be defined as `void` functions taking a scalar to write into an element, for example:
[source,c++]
----
template <int I>
...
----
Boost QVM automatically detects which alternative is provided.
--
...
- `write_element`: the expression `quat_traits<Quaternion>::write_element<I>(q)` returns mutable reference to the `I`-th 
element of `q`.

TIP: Alternatively, `write_element` may be defined as `void` function that takes (as the last argument) a scalar to write into the specified element. QVM automatically detects this case by examining the signature of the `write_element` template.
...
TIP: Alternatively, `write_element` and `write_element_idx` may be defined as `void` functions that take (as the last argument) a scalar to write into the specified element. QVM automatically detects this case by examining the signature of the `write_element` template.
